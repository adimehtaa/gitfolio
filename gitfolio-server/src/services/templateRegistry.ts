// gitfolio-server/src/services/templateRegistry.ts
...
class TemplateRegistry {
        ...
        for (const dir of dirs) {
            ...
            for (const file of files) {
                try {
                    ...
                    // Use file:// URL for ESM dynamic import on Windows + Linux
    ...
    getAllFree(): Template[] { return this.getAll().filter(t => t.meta.tier === 'free') }
    getAllPremium(): Template[] { return this.getAll().filter(t => t.meta.tier === 'premium') }
    getMeta(): TemplateMeta[] { return this.getAll().map(t => t.meta) }
